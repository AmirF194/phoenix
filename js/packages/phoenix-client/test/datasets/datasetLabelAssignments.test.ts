...
import {
  ...
  replaceDatasetLabelsForDataset,
} from "../../src/datasets";
...
describe("dataset label assignments", () => {
  it("lists labels using either a dataset name or GlobalID", async () => {
    ...
    const labelsByName = await listDatasetLabelsForDataset({
      client,
      dataset: { datasetName: "support-eval" },
    });
    const labelsById = await listDatasetLabelsForDataset({
      client,
  ...
  it("adds a label idempotently and returns the applied label", async () => {
    ...
    await expect(addDatasetLabelToDataset(params)).resolves.toEqual(
      goldenLabel
    );
    await expect(addDatasetLabelToDataset(params)).resolves.toEqual(
      goldenLabel
  ...
  it("removes a label idempotently", async () => {
    ...
    await expect(
      removeDatasetLabelFromDataset(params)
  ...
  it("replaces the complete label set", async () => {
    ...
    const labels = await replaceDatasetLabelsForDataset({
      client: createTestClient(),
      dataset: { datasetName: "support-eval" },
