...
export interface ListDatasetLabelsForDatasetParams extends ClientFn {
  /** The dataset to list labels for, selected by name or GlobalID. */
  dataset: DatasetSelector;
}

export interface AddDatasetLabelToDatasetParams extends ClientFn {
...
export interface RemoveDatasetLabelFromDatasetParams extends ClientFn {
...
export interface ReplaceDatasetLabelsForDatasetParams extends ClientFn {
 ...
 */
export async function listDatasetLabelsForDataset({
  client: _client,
  dataset,
}: ListDatasetLabelsForDatasetParams): Promise<DatasetLabel[]> {
 ...
 * @requires Phoenix server >= 17.16.0
 */
export async function addDatasetLabelToDataset({
  client: _client,
  dataset,
  labelId,
}: AddDatasetLabelToDatasetParams): Promise<DatasetLabel> {
  const client = _client ?? createClient();
 ...
 * @requires Phoenix server >= 17.16.0
 */
export async function removeDatasetLabelFromDataset({
  client: _client,
  dataset,
  labelId,
}: RemoveDatasetLabelFromDatasetParams): Promise<void> {
  const client = _client ?? createClient();
 ...
 * @requires Phoenix server >= 17.16.0
 */
export async function replaceDatasetLabelsForDataset({
  client: _client,
  dataset,
  labelIds,
}: ReplaceDatasetLabelsForDatasetParams): Promise<DatasetLabel[]> {
  const client = _client ?? createClient();
